<script lang="ts">
    ...
    async function onSearch() {
        ...
        loading_state = false;
        let cg = await HttpGet(
            ModrinthSearchURL + `&query=${encodeURIComponent(searchText)}`,
            "",
        );
        if (cg != "") {
            ...
            for (let i = 0; i < json.length; i++) {
                search_mod_cache.update((value) => {
                    value.push({
                        ...
                        download_count:
                            json[i].downloads > 10000
                                ? Math.floor(json[i].downloads / 10000) + "万"
                                : json[i].downloads,
...
>
    <MySelectCard title="">
        <div class="search-box">
            <MyTextInput
                style_in="border: none; outline: none; flex: 1; height: 50px; font-size: 20px;"
                on:blur={onSearchBoxInput}
                placeholder="搜索 Mod~"
            />
            <MyNormalButton
                style_in="width: 100px; height: 50px; border: none; cursor: pointer"
            ...
            >
                <svg
                    ...
                    style:stroke={$dark_mode ? "#f6f6f6cf" : "#1a1a1acf"}
                >
                    <path
                        d="M15 9.5a5.5 5.5 0 1 1-11 0a5.5 5.5 0 0 1 11 0Z M20 20l-6-6"
                    />
                </svg>
            </MyNormalButton>
        </div>
