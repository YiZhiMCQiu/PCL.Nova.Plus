...
<div
    id="progress-bar"
    style="{style_in}; --hov-percent-left: {(value / max) *
        100}%; --hov-percent-right: {1 -
        (value / max) * 100}%; --thumb-left: {Math.floor(
        (value / max) * 100,
    )}%;"
...
>
    <div
        ...
        on:mousedown|stopPropagation={thumbSlide}
    >
...
<style>
    ...
    #progress-bar:hover #progress {
        background: linear-gradient(
            to right,
            rgb(12, 92, 180) var(--hov-percent-left),
            rgb(185, 201, 218) var(--hov-percent-right)
        );
    }
    ...
    #progress {
        ...
        background: linear-gradient(
            to right,
            rgb(18, 122, 225) var(--hov-percent-left),
            rgb(215, 231, 248) var(--hov-percent-right)
        );
        transition: all 0.2s;
    }
    #bar {
