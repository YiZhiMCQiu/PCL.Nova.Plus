<script lang="ts">
    ...
    import { homepage_cache } from "../../store/changeBody";
    // 检测 主页缓存 的变化。
    const unsubscribe_homepage_cache = homepage_cache.subscribe(
        async (value) => {
            if (value == "") {
    ...
    async function recursion(xml: string): Promise<boolean> {
        try {
            structures = parseXmlToJson(
                `<?xml version="1.0" encoding="UTF-8"?><NovaHomePage>${xml.replaceAll('<?xml version="1.0" encoding="UTF-8"?>', "")}</NovaHomePage>`,
            )[0].children;
            if (!structures) {
                structures = [];
            }
