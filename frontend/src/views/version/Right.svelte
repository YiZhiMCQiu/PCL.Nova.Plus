<script lang="ts">
    ...
    async function selectMCVersion(num: number) {
        ...
        let rp = num <= 0 ? await GetCurrentMinecraftDir() : c_select_mc_length[num - 1].path
        fpath = rp
        ...
        if(paths.length == 0){
            ...
            for(let i = 0; i < paths.length; i++) {
                ...
                let p3 = p1 > 1 ? paths[i].substring(p1 + 1) : p2 > 0 ? paths[i].substring(p2 + 1) : ""
                height += 50
                select_mc_version.set([...$select_mc_version, {
                    path: paths[i],
                    name: p3
                }])
            }
    ...
    async function selectOneVersion(num: number) {
        await WriteConfig(await GetConfigIniPath(), "MC", "SelectVer", num.toString())
    ...
    {#if isTransitioning}
        <div id="transitioning" in:slide out:slide on:outroend={control_leave}>
            <div id="version-name">
                ...
                {#if show}
                    <div class="loading">
                        <MyLoadingPickaxe state={true} loading_text="未找到任何版本，请下载一个先吧~"/>
                    </div>
                {:else}
                    <div class="scroll-div">
                        <MySelectCard title="MC 实例" in_style="margin-top: 1px" maxHeight={height}>
                            <div class="version-all">
