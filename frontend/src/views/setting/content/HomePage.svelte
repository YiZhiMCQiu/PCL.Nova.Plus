<script lang="ts">
    ...
    import {GetConfigIniPath, ReadConfig, WriteConfig} from "../../../../wailsjs/go/launcher/ReaderWriter";
    ...
    import {GenerateTutorialHomePage, GetAllHomePage} from "../../../../wailsjs/go/launcher/MainMethod";
    ...
    async function selectAnyHomePage(index: number) {
        ...
        await WriteConfig(await GetConfigIniPath(), 'Misc', 'SelectHomePage', index.toString())
    }
    async function onlineTextInput(event: CustomEvent) {
        ...
        homepage_cache.set("")
        await WriteConfig(await GetConfigIniPath(), 'Misc', 'HomePageURL', text)
    }
    ...
    async function anyHomePageClick(index: number) {
        ...
        homepage_cache.set("")
        await WriteConfig(await GetConfigIniPath(), 'Misc', 'HomePageValue', index.toString())
    }
    async function loadingHomePage() {
        if ($select_homepage == 1) {
            ...
            loading_state = false
            setTimeout(() => {
                loading_text = "咱们还不支持预设主页喵~你可以加载一下本地主页或者联网更新！~"
                loading_state = true
            }, 1000)
            // TODO: 预设主页
    ...
    onMount(async () => {
        ...
        if($current_homepage < 0) {
            let index = Number(await ReadConfig(await GetConfigIniPath(), "Misc", "HomePageValue"))
            current_homepage.set(Number(index >= 0 ? index : -1))
        }
        if($homepage_url == "") {
            let url = await ReadConfig(await GetConfigIniPath(), "Misc", "HomePageURL")
...
>
    <MySelectCard title="主页设置">
        <div class="homepage-radio">
            <MyRadioButton isChecked={$select_homepage === 0} on:click={() => selectAnyHomePage(0)}>空白</MyRadioButton>
            <MyRadioButton isChecked={$select_homepage === 1} on:click={() => selectAnyHomePage(1)}>预设</MyRadioButton>
            <MyRadioButton isChecked={$select_homepage === 2} on:click={() => selectAnyHomePage(2)}>读取本地文件</MyRadioButton>
            <MyRadioButton isChecked={$select_homepage === 3} on:click={() => selectAnyHomePage(3)}>联网更新</MyRadioButton>
        </div>
    </MySelectCard>
                ...
                <MySelectCard title="预设主页">
                    <div class="version-all">
                        {#each $homepage_list as phl, i}
                            <div
                                    class="a-homepage"
                                    on:click={() => {anyHomePageClick(i)}}
                                    on:keydown|preventDefault
                                    style={$current_homepage === i ? 'cursor: default' : 'cursor: pointer'}
                            >
                                <MyRadioButton isChecked={i === $current_homepage} style_in="margin-left: 5px"/>
                                <img src={RedstoneLampOn} alt="主页" class="a-homepage-icon">
                                <div class="info" style="display: flex; flex-direction: column; margin-left: 10px; pointer-events: none">
                                    <MyNormalSpan>{phl.name}</MyNormalSpan>
                                    <MyNormalSpan style_in="font-size: 14px; color: gray;">{phl.path}</MyNormalSpan>
                                </div>
                            </div>
                        {/each}
                    </div>
        ...
        {#if isTransitioning && isSetting}
            ...
            >
                <MySelectCard title="本地主页">
                    <div class="version-all">
                        <div class="hint blue-hint" style="width: calc(100% - 30px);">
                            从 PCL.Nova/homepage/&lt;文件名&gt;.nxml 读取主页内容。<br>
                            你可以手动编辑该文件，向主页添加文本、图片、常用网站、快捷启动等功能。
                        </div>
                        <div class="yellow-hint hint" style="width: calc(100% - 30px);">
                            请谨慎使用陌生人提供的主页，恶意代码可能会造成损害！
                        </div>
                        <div style="margin-top: 5px; margin-bottom: 5px;">
                            <MyNormalButton style_in="width: 100px; height: 40px;" on:click={() => {homepage_cache.set(""); current_view.set("home")}}>刷新主页</MyNormalButton>
                            <MyNormalButton style_in="width: 100px; height: 40px; margin-left: 20px;" on:click={genTutorialHomePage}>生成教学主页</MyNormalButton>
                            <MyNormalButton style_in="width: 100px; height: 40px; margin-left: 20px;" on:click={() => { (async function xx(){
                                await messagebox("主页自定义教程", "1. 点击 生成教学主页 按钮，这会在 PCL.Nova/homepage/ 文件夹下生成 Custom.nxml 布局文件。<br>2. 使用记事本等工具打开这个文件并进行修改，修改完记得保存。<br>3. 点击刷新主页按钮，查看主页现在长啥样了。<br><br>你可以在生成教学文件后直接刷新主页，对照着进行修改，更有助于理解。")
                            })() }}>查看教程</MyNormalButton>
                        </div>
                        {#each $homepage_list as phl, i}
                            <div
            ...
            <MySelectCard title="联网主页">
                <div class="version-all">
                    ...
                    <div style="display: flex; align-items: center; margin-top: 5px;">
                        <div>下载地址</div>
                        <div style="flex: 1; margin-left: 50px"><MyTextInput style_in="height: 30px; width: calc(100% - 30px);" bind:value={onlineValue} on:blur={onlineTextInput} /></div>
                    </div>
                </div>
            </MySelectCard>
        </div>
