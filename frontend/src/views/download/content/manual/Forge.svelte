<script lang="ts">
    ...
    import {forge_list, forge_support} from "../../../../store/mc";
    ...
    onMount(async () => {
        if($forge_support.length <= 1) {
            ...
            loading_state = false
            let meta = await HttpGet("https://maven.minecraftforge.net/net/minecraftforge/forge/maven-metadata.xml", "")
            if(meta == "") {
        ...
        Object.keys($forge_list).forEach((name) => {
            temp_forge_support.push({
                version: name,
                height: $forge_list[name].length * 50 + 10
            })
    ...
    function onTopDiv() {
        let target = document.getElementsByClassName("component-forge")[0] as HTMLElement
